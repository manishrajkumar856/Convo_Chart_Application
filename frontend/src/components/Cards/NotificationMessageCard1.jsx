...
const NotificationMessageCard1 = ({ data }) => {
  ...
  const reject_handler = async (event) => {
    try {
      const response = await axios.post(
        `https://convo-chart-application-1.onrender.com/api/reject_request/${data.senderId}`,
        {
          userId: userData._id,
        },
        {
  ...
  const accept_handler = async (event) => {
    ...
    try {
      const response = await axios.post(
        `https://convo-chart-application-1.onrender.com/api/accept_request/${userData._id}`,
        {
  ...
  return (
    <div className="w-full  px-5 py-3 bg-[#ffff] rounded-2xl flex gap-10">
