...
const VerifyForm = () => {
  ...
  const handleSubmitHandler = async (event) => {
    ...
    try {
      const response = await axios.post(
        "https://convo-chart-application-1.onrender.com/api/user/auth/verify",
        {
          otp: getOtp,
          email: email,
        },
  ...
  const handleCancleBtn = async (event)=>{
    ...
    try {
        const response = await axios.post(
        "https://convo-chart-application-1.onrender.com/api/user/auth/cancle",
        {
          email: email,
        },
      );
  ...
  return (
    ...
    >
      <div className="w-full py-3  border-b border-[#c2bbbbc3]">
        <h3 className="text-2xl px-10 md:text-2xl font-medium text-start">
          Enter Security Code
        </h3>
      </div>
      {errorMessage && (
      ...
      <div className="w-full py-3 ">
        <p className="text-xl px-10 md:text-[1.1em] text-[#454343] text-start font-medium ">
          Please Check your email for a message with your otp. your otp is 4
          numbers long.
        </p>
      </div>
      <div className="w-full py-1 px-10 flex gap-5">
      ...
      <div className="flex mt-3 w-full justify-between items-center px-10 border-t py-2 border-[#b8b5b5a1]">
        <div className="font-semibold  text-[#436097] px-8 text-sm py-2 ">
          <Link to="/signup">Didn't get a code</Link>
        </div>
        <div className="flex gap-3">
          <button type="button" onClick={handleCancleBtn} className="active:scale-95 text-xl  md:text-xl  bg-[#b0b4b7] text-white px-8 rounded-2xl py-2 ">
            Cancle
          </button>

          <button type="submit" className="active:scale-95 text-xl  md:text-xl bg-[#549bdd] text-white px-8 rounded-2xl py-2 ">
            Continue
          </button>
        </div>
      </div>
